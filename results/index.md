...
- 2026-06-02-0144-eval-pfcud-fsc147-test :: eval :: FSC-147 test (full, n=1190) :: PF-CUD (blob + DINOv2 + rank-fusion + MST/Otsu + MDL-merge + hypothesis ranking, prior-free) :: status=aborted :: top1_MAE=n/a :: aborted at ~65/1190 (5.5%) to pivot to method R&D (see run README). Findings: (1) perf bottleneck was CPU color feature (rgb2lab per-candidate, 23.8s/img) — vectorized to 0.18s (×130, identical values); (2) grouping is fine but ranking fails — top1 collapses to ~2-4 while oracle MAE≈9.8 (correct group exists, ranking won't pick it; consistency cues bias toward tiny groups); (3) user wants an *adaptive, policy-free* selector — prototyped MDL model-selection (pf_cud/select/mdl_count.py) but it currently fails (noise-blob mega-cluster dominates "compression gain"); next: add DINOv2 visual to the MDL selector. env: venv /home/gaoyiyang/venvs/fsc, torch 2.6.0+cu124, GPU 4/5/6/7.
- 2026-06-02-2124-eval-pfcud-fsc147-plateau :: eval :: FSC-147 test (full, n=1190) :: PF-CUD scale-layer count (NEW coarsest_plateau scale-selection rule) :: status=done :: scale_MAE=44.02 :: parameter-free scale-layer selection most_stable→coarsest_plateau; full GPU eval scale MAE 47.37→44.02 (RMSE 131.91→129.95), gains on gt 1-10 (32→24) and 11-50 (25→19), 51-200 slightly up (49→53), 201+ still the bottleneck (310). scale remains best non-cheating predictor (top1 63.79, select 82.95, oracle 25.09; oracle_scale ceiling 15.64). thermal safe: 0 cooldowns, peak 39C. env: venv /home/gaoyiyang/venvs/fsc, torch 2.6.0+cu124, GPU 4/5/6/7.

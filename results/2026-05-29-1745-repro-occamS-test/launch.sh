#!/bin/bash
# OCCAM-S paper reproduction (FULL) on FSC-147 test 1/3 (n=396).
# Paper config (Spanakis et al., Table 1/8): single-class, bbox 224x224,
# dense seed-grid predictor (spacing=10), IoU=0.1 dedup, 3x3 multiscale ON,
# area-window mask processing (P0; min=5e-4, max=0.5), custom FINCH
# thresholds 12/9/7.75, pred=total.
# Target (paper FSC-147 test, <=300 objects): MAE 11.29 / RMSE 25.17.
# Target (paper FSC-147 test, full):           MAE 16.92 / RMSE 110.83.
set -uo pipefail
RUN=/home/czp/ws_yiyang/GOC-Freely-Object-Counting/results/2026-05-29-1745-repro-occamS-test
